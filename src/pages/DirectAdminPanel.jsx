...
function DirectAdminPanel() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      // Poll every 500ms for updates (more frequent than before)
      const interval = setInterval(() => {
        loadData();
      }, 500);
      
      // Listen for direct updates
      const handleDirectUpdate = () => {
        console.log('🔄 Direct update received');
        loadData();
      };
      ...
      window.addEventListener('directOrdersUpdate', handleDirectUpdate);
      window.addEventListener('storage', handleStorageChange);
      
      return () => {
        clearInterval(interval);
        window.removeEventListener('directOrdersUpdate', handleDirectUpdate);
  ...
  const loadData = () => {
    try {
      setIsLoading(true);
      const freshOrders = DirectSync.getOrders();
      setOrders(freshOrders);
      
      const revenue = DirectSync.getTotalRevenue();
  ...
  const updateStatus = (orderId, newStatus) => {
    DirectSync.updateOrderStatus(orderId, newStatus);
    loadData();
  };

  const rejectOrder = (orderId) => {
    if (confirm('❌ Tem certeza que deseja recusar este pedido?')) {
      DirectSync.rejectOrder(orderId, 'Pedido recusado pela loja');
      loadData();
    }
  };
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <motion.div 
          ...
          className="bg-gradient-to-br from-gray-900 to-black border-2 border-yellow-500/30 rounded-2xl p-6 md:p-8 w-full max-w-md"
        >
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto p-4 md:p-8">
        ...
        {DirectSync.getHistoricalRevenue() > 0 && (
          <motion.div
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-0">
              <div>
                ...
                <p className="text-2xl md:text-3xl font-black text-white">R$ {DirectSync.getHistoricalRevenue().toFixed(2).replace('.', ',')}</p>
              </div>
...
export default DirectAdminPanel;